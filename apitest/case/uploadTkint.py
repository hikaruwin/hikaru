...
from upload_pg import uploadIPA, IpaCreatedLastly
...
class WidgetsDemo:
    ...
    def __init__(self):
        ...
        btuploadIPA = Button(frame1, text=u'上传', command=self.uploadIPA)
        btuploadIPA.pack()
        global m
        # m = Label(frame1, highlightcolor='yellow', text='')
        m = Text(frame1, height=5)
        m.pack()
    ...
    def uploadIPA(self):
        ...
        r = uploadIPA(IpaCreatedLastly(IpaPath))
        print r
        message = r[u'message']
        if message == '':
        # print error
            appQRCodeURL = str(r['data']['appQRCodeURL'])
            # m['text'] = u'操作成功，app地址是:'+appQRCodeURL
            m.insert(END, u'上传成功，app地址是:'+appQRCodeURL)
        else:
            # m['text'] = u'操作失败：'+message
            m.insert(END, u'上传失败：'+message)
